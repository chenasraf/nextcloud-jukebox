      ...
      <div
        ...
        <div v-if="queue.length > 0" class="queue-list">
          <ul v-for="(media, index) in queue">
            <TrackListItem
              v-if="media.type == 'track'"
              :key="'track-' + media.id"
              :media="media as unknown as Track"
              mediaType="track"
              @play="onPlay(media)"
              disable-play-next
            ...
            <PodcastEpisodeListItem
              ...
              :key="'podcast-' + media.id"
              :episode="media as unknown as PodcastEpisode"
              @play="onPlay(media)"
              disable-play-next
              disable-add-to-queue>
              <template #actions-end>
            ...
            <RadioStationListItem
              v-else-if="media.type == 'radio'"
              :key="'radio-' + media.id"
              :station="media as unknown as RadioStation"
              @play="onPlay(media)"
              disable-play-next
              disable-add-to-queue>
              <template #actions-end>
